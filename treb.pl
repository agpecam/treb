...
my $VALID_TIMESTAMP = qr/^(Feb ( [1-9]|[12]\d)|(Apr|Jun|Sep|Nov) ( [1-9]|[12]\d|30)|(Jan|Mar|May|Jul|Aug|Oct|Dec) ( [1-9]|[12]\d|3[01])) ([0-1]\d|2[0-3]):[0-5]\d:[0-5]\d$/;
...
# t_dlm(T_HOUR) = ':'
sub t_dlm($) {
	my $t = shift;		# hereinafter '$t' - term index - 0 for months, 1 for days, ..., 4 for seconds
...
sub t_str($$) {
	my ($ts, $t) = @_;	# hereinafter '$ts' - timestamp string in a form of 'Mmm dd hh:mm:ss'
...
sub t_tos($$) {
	...
	return $t != T_MONTH ? ($val < 10 ? ($t == T_DAY ? " $val" : "0$val") : "$val") : $rmonth{$val};
}
...
sub t_mod($$) {
	...
	return $t > T_MONTH ? ($t < T_MINUTE ? ($t == T_DAY ? $dmonth[t_int($ts, T_MONTH)] : 24): 60) : 12;
}
...
sub t_dec($$) {
	...
	return $tn . ($t < T_SECOND ? substr(ts_max($tn, $t + 1), ($t + 1) * 3 + 1) : S_EMPTY);	
}
...
sub t_inc($$) {
	...
	my $tn = t_tos((t_int($ts, $t) + 1 - ($t == T_DAY ? 1 : 0)) % t_mod($ts, $t) + ($t == T_DAY ? 1 : 0), $t);
	$tn = substr($ts, 0, $t * 3 + ($t > T_MONTH ? 1 : 0)) . $tn . t_dlm($t);

	return $tn . ($t < T_SECOND ? substr(ts_min($tn, $t + 1), ($t + 1) * 3 + 1) : S_EMPTY);	
}
...
sub ts_min($$) {
	...
	return substr($ts, 0, $t * 3 + ($t > T_MONTH ? 1 : 0)) . substr(t_str($ts, 0) . '  1 00:00:00', $t * 3 + ($t > T_MONTH ? 1 : 0));
}
...
sub ts_max($$) {
	...
	my $mnt = $t == T_MONTH ? (t_int($ts, T_MONTH) + 11) % 12 : t_int($ts, T_MONTH);
	my $mts = $rmonth{$mnt} . C_SPACE . $dmonth[$mnt] . ' 23:59:59';

	return substr($ts, 0, $t * 3 + ($t > T_MONTH ? 1 : 0)) . substr($mts, $t * 3 + ($t > T_MONTH ? 1 : 0));
}
...
sub ts_tail($$) {
	...
	return $t > T_MONTH ? ($t <= T_SECOND ? substr($ts, $t * 3 + 1) : S_EMPTY) : $ts;
}
...
sub t_digit_range_regex($$;$) {
	...
	return S_EMPTY	if defined $dscope and $d1 eq @$dscope[0] and $d2 eq @$dscope[1];
...
sub t_range_regex($$$) {
	...
	return S_EMPTY if ts_tail($ts1, $t) eq ts_tail(ts_min($ts1, $t), $t) and ts_tail($ts2, $t) eq ts_tail(ts_max($ts1, $t), $t);

	$t1 = t_str($ts1, $t); $t2 = t_str($ts2, $t);
	...
	if ($t == T_MONTH) {
		...
		$t1H = substr($t1, 0, 1); $t1L = substr($t1, 1, 1);
		$t2H = substr($t2, 0, 1); $t2L = substr($t2, 1, 1);
		...
		if (($r1 = t_digit_range_regex($ds2[0], $t2L, \@ds2)) ne S_EMPTY) {
			push @rs, $r2 if ($r2 = t_digit_range_regex($t1H, sprintf("%d", int($t2H) - 1))) ne S_EMPTY;
			push @rs, "$t2H" . "$r1";
		} else {
...
# ts_regex(TS_LEFT,  'Jan 19 16:25:11') = 'Jan (19 (16:(25:(1[1-9]|[2-5])|2[6-9]|[3-5])|1[7-9]|2)|[23])'
# ts_regex(TS_RIGHT, 'Jan 19 16:25:11') = 'Jan (19 (16:(25:(0|1[01])|[01]|2[0-4])|0|1[0-5])| |1[0-8])'
sub ts_regex($$;$$);
...
sub ts_regex($$;$$) {
	...
	return S_EMPTY if $t <= T_SECOND - $ulevel and ts_tail($ts, $t + $ulevel) eq ts_tail($tside == TS_LEFT ? ts_min($ts, $t + $ulevel) : ts_max($ts, $t + $ulevel), $t + $ulevel);
...
sub ts_range_regex($$) {
	...
	for ($t = T_MONTH; $t <= T_SECOND and t_str($ts1, $t) eq t_str($ts2, $t); $t++) {
		$rs .= t_str($ts1, $t) . t_dlm($t);	
	}

	($t == T_MONTH or $t > T_SECOND or t_int($ts1, $t) < t_int($ts2, $t)) or die "ERROR! '$ts1' > '$ts2'\n";

	if ($t <= T_SECOND) {
		$ts1 = t_inc($ts1, $t) if ($rl = ts_regex(TS_LEFT , $ts1, $t)) ne S_EMPTY;
		$ts2 = t_dec($ts2, $t) if ($rr = ts_regex(TS_RIGHT, $ts2, $t)) ne S_EMPTY;
...
(scalar @ARGV == 2) or die "Usage: treb.pl timestamp_begin timestamp_end, for example: treb 'Jan  1 00:00:00' 'Jul  3 23:59:59'\n";
